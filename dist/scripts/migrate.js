...
const auditDir = (0, path_1.join)(process.cwd(), "migration_logs");
const ensureAuditDir = () => {
    if (!(0, fs_1.existsSync)(auditDir)) {
        (0, fs_1.mkdirSync)(auditDir, { recursive: true });
    }
};
const auditLog = (name, payload) => {
    ensureAuditDir();
    const file = (0, path_1.join)(auditDir, `${name}.jsonl`);
    const line = JSON.stringify(payload);
    (0, fs_1.writeFileSync)(file, `${line}\n`, { encoding: "utf8", flag: "a" });
};
const parseArgs = () => {
...
const shouldRun = (name, options) => {
    if (options.only.size > 0 && !options.only.has(name))
        return false;
    if (options.skip.size > 0 && options.skip.has(name))
        return false;
    return true;
};
...
const pickCollection = (collections, names) => {
...
const promotionIdMap = new Map();
const promotionCardIdMap = new Map();
const adIdMap = new Map();
const designRequestIdMap = new Map();
const getMappedId = (map, legacyId) => {
    if (!legacyId)
        return undefined;
...
const ensureSystemUser = async (options) => {
    if (options.dryRun)
        return -1;
    if (systemUserId)
        return systemUserId;
...
const ensureEmptyDatabase = async (options) => {
    if (options.dryRun)
        return;
    if (options.force)
        return;
...
const preloadUserMapFromPostgres = async () => {
    ...
    for (const row of existing) {
        if (row.legacy_id)
            userIdMap.set(row.legacy_id, row.id);
    }
...
const preloadUserMapFromMongoForDryRun = async (db, collections) => {
...
const preloadCardMapFromPostgres = async () => {
    ...
    for (const row of existing) {
        if (row.legacy_id)
            cardIdMap.set(row.legacy_id, row.id);
    }
    if (existing.length > 0) {
...
const preloadVoucherMapFromPostgres = async () => {
    ...
    for (const row of existing) {
        if (row.legacy_id)
            voucherIdMap.set(row.legacy_id, row.id);
    }
    if (existing.length > 0) {
        log.info(`Preloaded ${existing.length} vouchers from Postgres for mapping.`);
    }
};
const preloadVoucherMapFromMongoForDryRun = async (db, collections) => {
...
const preloadGroupMapFromPostgres = async () => {
    ...
    for (const row of existing) {
        if (row.legacy_id)
            groupIdMap.set(row.legacy_id, row.id);
    }
    if (existing.length > 0) {
        log.info(`Preloaded ${existing.length} groups from Postgres for mapping.`);
    }
};
const preloadGroupMapFromMongoForDryRun = async (db, collections) => {
...
const preloadGroupSessionMapFromPostgres = async () => {
    ...
    for (const row of existing) {
        if (row.legacy_id)
            groupSessionIdMap.set(row.legacy_id, row.id);
    }
    if (existing.length > 0) {
        log.info(`Preloaded ${existing.length} group sessions from Postgres for mapping.`);
    }
};
const preloadGroupSessionMapFromMongoForDryRun = async (db, collections) => {
...
const preloadPromotionCardMapFromMongoForDryRun = async (db, collections) => {
...
const ensurePromotionCardFromDoc = async (doc, options) => {
    const promotionLegacyId = idToString(doc._id);
    if (!promotionLegacyId)
        return undefined;
...
const ensureGroupSessionFromDoc = async (doc, options, legacyIdOverride) => {
    ...
    const adminId = sanitizeString(doc.adminId || doc.admin_id || doc.admin || doc.fromUserId || doc.senderId) || "unknown";
    const adminName = sanitizeString(doc.adminName || doc.admin_name || doc.adminUserName || doc.senderName) || "Unknown";
    const adminPhone = sanitizeString(doc.adminPhone || doc.admin_phone || doc.senderPhone) || "NA";
...
const migrateUsers = async (db, collections, options) => {
    ...
    for await (const doc of cursor) {
        ...
        if (options.dryRun) {
            if (legacyId)
                userIdMap.set(legacyId, -(count + 1));
            count += 1;
            continue;
        }
        try {
...
const migrateProfiles = async (db, collections, options) => {
    if (!shouldRun("profiles", options))
        return;
    const name = pickCollection(collections, ["users", "user"]);
    if (!name)
        return;
    ...
    for await (const doc of cursor) {
        ...
        const hasAny = data.full_name || data.phone || data.avatar_url || data.location || data.bio;
        if (!hasAny)
            continue;
        if (options.dryRun) {
            count += 1;
...
const migrateUserRoles = async (db, collections, options) => {
    if (!shouldRun("user_roles", options))
        return;
    const name = pickCollection(collections, ["users", "user"]);
    if (!name)
        return;
    ...
    for await (const doc of cursor) {
        ...
        let role = client_1.Role.customer;
        if (roleValue.includes("admin") || toBoolean(doc.isVoucherAdmin, false))
            role = client_1.Role.admin;
        if (roleValue.includes("business"))
...
const migrateCategories = async (db, collections, options) => {
    if (!shouldRun("categories", options))
        return;
...
const migrateCards = async (db, collections, options) => {
    ...
    for await (const doc of cursor) {
        ...
        try {
            ...
            if (legacyId)
                cardIdMap.set(legacyId, created.id);
...
const migrateBusinessPromotions = async (db, collections, options) => {
    ...
    for await (const doc of cursor) {
        ...
        if (!userId) {
            ...
            if (missingUser <= 5) {
                log.warn(`BusinessPromotion ${legacyId} missing user. userId=${idToString(doc.userId)} Using system user.`);
            }
        }
        const data = {
            legacy_id: legacyId ?? undefined,
...
const migrateVouchers = async (db, collections, options) => {
    ...
    for await (const doc of cursor) {
        ...
        try {
            ...
            if (legacyId)
                voucherIdMap.set(legacyId, created.id);
...
const migrateVoucherRedemptions = async (db, collections, options) => {
    if (!shouldRun("voucherredemptions", options))
        return;
...
const migrateVoucherTransferLogs = async (db, collections, options) => {
    if (!shouldRun("vouchertransferlogs", options))
        return;
...
const migrateContacts = async (db, collections, options) => {
    if (!shouldRun("contacts", options))
        return;
...
const migrateNotifications = async (db, collections, options) => {
    if (!shouldRun("notifications", options))
        return;
...
const migrateGroupSessions = async (db, collections, options) => {
...
const migrateCardShares = async (db, collections, options) => {
    if (!shouldRun("cardshares", options))
        return;
    ...
    for await (const doc of cursor) {
        if (options.limit && count >= options.limit)
            break;
        const legacyId = idToString(doc._id);
        const sessionLegacyId = idToString(doc.sessionId || doc.groupSessionId || doc.session || doc.groupSession) || `cardshare-${legacyId ?? Math.random().toString(36).slice(2, 8)}`;
        let sessionId = getMappedId(groupSessionIdMap, sessionLegacyId);
        if (!sessionId) {
            let sessionDoc = null;
            if (sessionCollection && mongodb_1.ObjectId.isValid(sessionLegacyId)) {
                sessionDoc = await sessionCollection.findOne({ _id: new mongodb_1.ObjectId(sessionLegacyId) });
            }
            sessionId = await ensureGroupSessionFromDoc(sessionDoc ?? {
                _id: sessionLegacyId,
...
const migrateSharedCards = async (db, collections, options) => {
    if (!shouldRun("sharedcards", options))
        return;
...
const migrateGroupSharedCards = async (db, collections, options) => {
    if (!shouldRun("groupsharedcards", options))
        return;
...
const migrateFeedbacks = async (db, collections, options) => {
    if (!shouldRun("feedbacks", options))
        return;
...
const migrateReviews = async (db, collections, options) => {
    if (!shouldRun("reviews", options))
        return;
    ...
    for await (const doc of cursor) {
        ...
        const legacyId = idToString(doc._id);
        let userId = getMappedId(userIdMap, idToString(doc.userId || doc.user || doc.user_id));
        const businessLegacyId = idToString(doc.businessId || doc.business || doc.business_id || doc.cardId || doc.card || doc.businessCardId);
        let businessId = getMappedId(cardIdMap, businessLegacyId);
        if (!businessId) {
            const promotionLegacyId = idToString(doc.businessPromotionId || doc.promotionId || doc.businessPromotion || doc.promotion || businessLegacyId);
            businessId = getMappedId(promotionCardIdMap, promotionLegacyId);
            if (!businessId && promotionLegacyId && promotionCollection) {
                const promoObjectId = mongodb_1.ObjectId.isValid(promotionLegacyId) ? new mongodb_1.ObjectId(promotionLegacyId) : null;
...
const migrateGroups = async (db, collections, options) => {
    ...
    for await (const doc of cursor) {
        ...
        if (!adminId) {
            ...
            if (allowAdminFallback && Array.isArray(doc.members)) {
                const fallbackLegacy = doc.members.map(idToString).find((v) => getMappedId(userIdMap, v));
        ...
        try {
            ...
            if (Array.isArray(doc.members)) {
                for (const member of doc.members) {
                    ...
                    if (!memberId)
                        continue;
                    const roleValue = typeof doc.memberRoles?.get === "function"
                        ...
                        await prisma.groupMember.create({
                            data: {
                                ...
                                is_muted: Array.isArray(doc.mutedBy)
                                    ? doc.mutedBy.some((v) => idToString(v) === idToString(member))
                                    : false,
                            },
                        });
                    }
...
const migrateChats = async (db, collections, options) => {
    if (!shouldRun("chats", options))
        return;
    ...
    for await (const doc of cursor) {
        ...
        try {
            ...
            if (legacyId)
                chatIdMap.set(legacyId, created.id);
            pendingLastMessage.push({
                legacyId: legacyId ?? String(created.id),
                lastMessageLegacyId: idToString(doc.lastMessage),
            });
            if (Array.isArray(doc.participants)) {
                for (const participant of doc.participants) {
                    ...
                    const isMuted = Array.isArray(doc.mutedBy)
                        ? doc.mutedBy.some((v) => idToString(v) === idToString(participant))
                        : false;
                    try {
...
const migrateMessages = async (db, collections, options) => {
    if (!shouldRun("messages", options))
        return;
    ...
    for await (const doc of cursor) {
        ...
        if (!senderId)
            continue;
        const messageTypeRaw = String(doc.messageType || "text").toLowerCase();
        const messageType = client_1.MessageType[messageTypeRaw] ?? client_1.MessageType.text;
        const data = {
            legacy_id: legacyId ?? undefined,
            sender_id: senderId,
        ...
        try {
            ...
            if (legacyId)
                messageIdMap.set(legacyId, created.id);
            count += 1;
        }
        catch (err) {
...
const migrateAds = async (db, collections, options) => {
    if (!shouldRun("ads", options))
        return;
    ...
    for (const [, cardId] of cardIdMap.entries()) {
        ...
        if (!card)
            continue;
        const phones = [card.phone, card.personal_phone, card.company_phone].filter(Boolean);
        for (const p of phones)
            phoneToCardId.set(normalizePhone(p) ?? p, cardId);
    }
    let systemAdsCardId = null;
    const ensureSystemAdsCard = async () => {
        if (options.dryRun) {
            return -1;
        }
        if (systemAdsCardId)
            return systemAdsCardId;
    ...
    for await (const doc of cursor) {
        ...
        const data = {
            ...
            ad_type: doc.adType === "video" ? client_1.AdType.inline : client_1.AdType.banner,
        ...
        try {
            ...
            if (legacyId)
                adIdMap.set(legacyId, created.id);
            count += 1;
        }
        catch (err) {
...
const migrate = async () => {
    ...
    const client = new mongodb_1.MongoClient(uriWithPreference);
